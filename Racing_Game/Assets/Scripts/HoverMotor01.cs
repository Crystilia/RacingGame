...
public class HoverMotor01 : MonoBehaviour {
	...
	void Update () {
        ...
        if (true)
        {
            transform.Rotate(0.0f, turnInput, 0.0f);
    ...
    private void FixedUpdate()
    {
        ...
        {
            speedHolder = Input.GetAxis("Vertical");
            crb.AddRelativeForce(0.0f, 0.0f, thrustInput);
        }else if(Input.GetAxis("Vertical") == 0 && Input.GetAxis("Horizontal") > 0 || Input.GetAxis("Horizontal") < 0)
        {
            Debug.Log(crb.velocity.magnitude);
            //crb.AddRelativeForce(0.0f, 0.0f, crb.velocity.magnitude, ForceMode.Impulse);
        }
        //crb.AddRelativeForce(0.0f, 0.0f, thrustInput * speed);
